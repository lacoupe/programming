...
def roulette(play_type,amount_played,**kwargs):
    ...
    if play_type.lower()=='number':
        n = random.randint(0,36)
        if n == number:
            amount_gained = amount_played * 35
...
if play_type.lower()=='color':
    roulette(play_type.lower(),amount_played,color = color_choice)
else:
    roulette(play_type.lower(),amount_played,number = number_choice)
